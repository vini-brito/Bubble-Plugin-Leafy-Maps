      ...
      $(window).on("keydown", event => {
        let keyNumber = getKeyNumber(event);
          ...
          if (keyNumber === 9) {
            ...
            instance.data.DRAW_TOGGLE = !instance.data.DRAW_TOGGLE;
            if (instance.data.DRAW_TOGGLE) {
              instance.canvas.css({'cursor': `url(${cursorUrl}), auto`});
            } else {
              instance.canvas.css({'cursor': ``});
              // console.log(instance.data.SAVED_POLYLINE);
             if(instance.data.SAVED_POLYLINE.length > 2) {
               const based = drawLine([instance.data.SAVED_POLYLINE[instance.data.SAVED_POLYLINE.length - 1], instance.data.SAVED_POLYLINE[0]]);
               if(instance.data.draw_city) {
                console.log(latLongs, 'remove');

                based.removeFrom(mymap);
                drawedObjects.forEach(el => el.removeFrom(mymap));
                
                const pol = L.polygon(instance.data.SAVED_POLYLINE, {
                  fill: true,
                  fillColor: 'gray',
                  stroke: true,
                  color: 'gray'
                ...
                const INDEX = CITY_AREAS.length;
                const input_test = document.createElement('input');
                input_test.setAttribute('value', NAME);
                pol.bindTooltip(NAME).openTooltip();
                const POPUP = L.popup().setContent(input_test).openPopup();

                
                input_test.addEventListener('input', e => {
                  console.log(e.target.value);
                  CITY_AREAS[INDEX].name = e.target.value;
                  pol.bindTooltip(e.target.value).openTooltip();
                });
                pol.bindPopup(POPUP).openPopup();
                pol.addTo(mymap);
                pol.on('click', e => {
                  // console.log(e, e.target);
                  console.log(e, e.target, e.target.parentElement);
                  const { selected } = CITY_AREAS[INDEX];
                  CITY_AREAS[INDEX].selected = !selected;
                  e.target.setStyle({
                    fillColor: selected ? 'gray' : 'rebeccapurple',
                    color: selected ? 'gray' : 'rebeccapurple',
                  });

                  // pol.bindTooltip('test').openTooltip();
                });

                CITY_AREAS.push({
                  name: NAME,
                  path: instance.data.SAVED_POLYLINE,
                  polyline: pol,
                  selected: false
                });
